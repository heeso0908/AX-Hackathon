...
def render_metrics(result: dict) -> None:
    ...
    cards = [
        ("우선 대응 목록", f"{summary['cs_immediate_count']:,}", "CS팀 우선 확인 필요"),
        ("이탈 위험", f"{summary['churn_risk_count']:,}", "불만/긴급도 기반 추적 필요"),
        ("제품 개선 후보", f"{summary['product_candidate_count']:,}", "제품팀 검토 대상"),
        ...
        ("기능 요청", f"{count_by_type(summary, '기능 요청'):,}", "제품 개선 후보로 반영 필요"),
...
def render_workflow() -> None:
    steps = [
        ...
        ("3", "CS 분류", "불만, 기능 요청, 칭찬, 일반 문의로 1차로 나눠 정리합니다."),
        ("4", "이슈 집계", "토픽, 긴급도, 고객군 기준으로 반복 패턴을 묶습니다."),
        ("5", "운영 인사이트", "즉시 대응 건과 제품 전달 이슈를 분리해 정리합니다."),
        ("6", "후속 액션", "CS 리포트와 제품 개선 후보 문서를 생성합니다."),
    ]
...
def render_proposals(result: dict) -> None:
    st.markdown(
        """
        <div class="dashboard-card">
            <div class="card-label">제품 개선 후보</div>
            <div class="card-title">제품팀 전달 후보</div>
            <div class="card-subtitle">반복 VoC를 기능/운영 개선 목록으로 묶고 우선순위 점수로 정렬했습니다.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with hero_left:
    st.markdown(
        ...
        <div class="hero">
            ...
            <div class="hero-desc">
                우선 대응 목록, 반복 이슈, 데이터 품질, 주간 리포트
            </div>
            <div class="hero-actions">
                <span class="hero-chip">우선 대응 목록</span>
